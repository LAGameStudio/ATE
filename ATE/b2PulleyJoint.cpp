...
void b2PulleyJoint::InitVelocityConstraints(const b2TimeStep& step)
{
	...
	float cr1u1 = b2Cross(r1, m_u1);
	float cr2u2 = b2Cross(r2, m_u2);
...
void b2PulleyJoint::SolveVelocityConstraints(const b2TimeStep& step)
{
	...
	if (m_state == e_atUpperLimit)
	{
		...
		float Cdot = -b2Dot(m_u1, v1) - m_ratio * b2Dot(m_u2, v2);
...
bool b2PulleyJoint::SolvePositionConstraints(float baumgarte)
{
...
float b2PulleyJoint::GetLength1() const
{
...
float b2PulleyJoint::GetLength2() const
{
