...
b2ContactSolver::b2ContactSolver(b2Contact** contacts, int contactCount,
								b2StackAllocator* allocator, float impulseRatio)
{
	...
	for (int i = 0; i < m_constraintCount; ++i)
	{
		...
		float friction = b2MixFriction(fixtureA->GetFriction(), fixtureB->GetFriction());
		float restitution = b2MixRestitution(fixtureA->GetRestitution(), fixtureB->GetRestitution());
		...
		for (int j = 0; j < cc->pointCount; ++j)
		{
			...
			float rnA = b2Cross(ccp->rA, cc->normal);
			float rnB = b2Cross(ccp->rB, cc->normal);
			rnA *= rnA;
			rnB *= rnB;
			...
			float rtA = b2Cross(ccp->rA, tangent);
			float rtB = b2Cross(ccp->rB, tangent);
			rtA *= rtA;
			rtB *= rtB;
			...
			float vRel = b2Dot(cc->normal, vB + b2Cross(wB, ccp->rB) - vA - b2Cross(wA, ccp->rA));
		...
		if (cc->pointCount == 2)
		{
			...
			float rn1A = b2Cross(ccp1->rA, cc->normal);
			float rn1B = b2Cross(ccp1->rB, cc->normal);
			float rn2A = b2Cross(ccp2->rA, cc->normal);
			float rn2B = b2Cross(ccp2->rB, cc->normal);
...
void b2ContactSolver::SolveVelocityConstraints()
{
	for (int i = 0; i < m_constraintCount; ++i)
	{
		...
		for (int j = 0; j < c->pointCount; ++j)
		{
			...
			float newImpulse = b2Clamp(ccp->tangentImpulse + lambda, -maxFriction, maxFriction);
		...
		if (c->pointCount == 1)
		{
			...
			float newImpulse = b2Max(ccp->normalImpulse + lambda, 0.0f);
...
// Sequential solver.
bool b2ContactSolver::SolvePositionConstraints(float baumgarte)
{
	...
	for (int i = 0; i < m_constraintCount; ++i)
	{
		...
		for (int j = 0; j < c->pointCount; ++j)
		{
			...
			float C = b2Clamp(baumgarte * (separation + b2_linearSlop), -b2_maxLinearCorrection, 0.0f);
