...
#define VERSION_STRING "ATE: ATLAS TILE EDITOR\nEARLY ACCESS\nVERSION 0.5"
...
void DrawLoading( GLImage *loading, HDC *game_dc, HWND *game_window, int w, int h ) {
 ...
 glScissor(0,0,w,h);//rView.w,rView.h); 
 ...
 glBindTexture( GL_TEXTURE_2D, loading->texture );
 
 glColor3f( 1.0f, 1.0f, 1.0f );
 glBegin(GL_POLYGON);
 glTexCoord2d(0.0,0.0); glVertex2i(0,0);
 glTexCoord2d(1.0,0.0); glVertex2i(w,0);
 glTexCoord2d(1.0,1.0); glVertex2i(w,h);
 glTexCoord2d(0.0,1.0); glVertex2i(0,h);
 glEnd();
...
#include "TwoDeeElementEditor.h"

#include "GameWorkspaceLoadingWindow.h"
...
// windows.Add(new SmokeTest); return;
 windows.Add(new GameWorkspaceLoadingWindow);
// windows.Add(new AtlasAssembler);
}