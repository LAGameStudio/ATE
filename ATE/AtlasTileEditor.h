...
#include "AtlasTilePNGExporter.h"
#include "AtlasTilesetExporter.h"
...
//  if ( !level1 && !level2 && !level3 && !level4 ) level4=true;
  drawSquare=fast.toggle("Draw Square",drawSquare,(int)area.x2+8,(int)area.y+144,20,20);  
  dropSquare=fast.toggle("Drop Square",dropSquare,(int)area.x2+8,(int)area.y+168,20,20);  
 ...
 void TileInspector() {
  ...
  Cartesiand buttons(area.x2+64,area.y+32);
  if ( fast.button("Bake to PNGs",(int)buttons.x,(int)buttons.y,192,24) ) {
   AtlasTilePNGExporter *atpe=new AtlasTilePNGExporter;
   atpe->back_to=this;
   this->visible=false;
   atpe->tile=edTile;
   atpe->atlas=ed->atlas;
  ...
  if ( fast.button("Bake Tileset",(int)buttons.x,(int)buttons.y+32,192,24) ) {
   AtlasTilesetExporter *atse=new AtlasTilesetExporter;
   atse->back_to=this;
   this->visible=false;
   atse->tileset=ed;
   atse->atlas=ed->atlas;
 ...
 void TileEdit() {
  ...
  if ( ( selectingHeightTile ) && edTile->name.length >0 && ed->name.length >0
    && fast.button(
...
//  art.Rectangle(crayons.Pick(black),area.x,area.y,area.w,area.h);
  ...
  if ( fast.button("EXPORT TILES",32+128,display->h-32,256,16) ) {
  }
 }
 void RenderEditor() {
 ...
 void RenderSelector() {
  ...
  int i=0;
  Cartesiand gridSelection;
  EACH(workspace->workspaceAtlasTilesets.first,AtlasTilesetHandle,tsh) {
   AtlasTileset *ts=tsh->p;
   art.Rectangle(crayons.Pick(black),sx,sy,dx,dy);
   if ( i < selectorScrolled ) { i++; continue; }
   bool hovering=WITHIN(input->mxi,input->myi,(int)sx,(int)sy,(int)(sx+dx),(int)(sy+dy));
