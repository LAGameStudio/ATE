 ...
 TOJSON {
  ...
  for ( int j=0; j<(int)h; j++ ) {
   ...
   for ( int i=0; i<(int)w; i++ ) {
    out+=(*this)(i,j).toString()+((i<(int)w-1)? "," : "");
   }
   out+=string("]");
   if (j<(int)h-1) out+=string(",\n");
   else out+=string("\n");
  }
  return out.value;
 }
...
//  OUTPUT("UPDATING AtlasTile -> Stashing tile:\n%s\n----\n",toString().c_str());
  triangles.Clear();
 ...
 void RenderNormal( Crayon tint, Atlas *in, double x, double y, double w, double h ) {
  if ( !in->normal ) return;
  if ( stashed_id > 0 && !art.stash.stashed[stashed_id]->replace ) {
   art.Element(in->normal,tint,transparency,stashed_id,x,y,w,h);
  }
 }
 void RenderNormal( Crayon tint, Blends b, Atlas *in, double x, double y, double w, double h ) {
  if ( !in->normal ) return;
  if ( stashed_id > 0 && !art.stash.stashed[stashed_id]->replace ) {
   art.Element(in->normal,tint,b,stashed_id,x,y,w,h);
  }
 }
 void RenderHeight( Crayon tint, Atlas *in, double x, double y, double w, double h ) {
  if ( !in->height ) return;
  if ( stashed_id > 0 && !art.stash.stashed[stashed_id]->replace ) {
   art.Element(in->height,tint,transparency,stashed_id,x,y,w,h);
  }
 }
 void RenderHeight( Crayon tint, Blends b, Atlas *in, double x, double y, double w, double h ) {
  if ( !in->height ) return;
  if ( stashed_id > 0 && !art.stash.stashed[stashed_id]->replace ) {
   art.Element(in->height,tint,b,stashed_id,x,y,w,h);
  }
 }
 KEYWORDS({
  TXTWORD("name",name)
 ...
 void PostLoad();
 TOJSON {
  Zstring filteredName=name; filteredName("\"","\\\"");
  Zstring filteredatlasTilesetName=atlasTilesetName; filteredatlasTilesetName("\"","\\\"");
  Zstring filteredatlasTileName=atlasTileName; filteredatlasTileName("\"","\\\"");
  return FORMAT(
   JSONSTR("name")
   JSONARRAY("L1") JSONARRAY("L2") JSONARRAY("L3") JSONARRAY("L4")
   JSONSTR("atSet")
   JSONSTR("atTile"),
   filteredName.c_str(),
   level1.toJSON().c_str(),
   ...
   level3.toJSON().c_str(),
   level4.toJSON().c_str(),
   filteredatlasTilesetName.c_str(),
   filteredatlasTileName.c_str()
  )
   +string("\"quads\":[\n")+quads.toJSON()+string("],\n")
   +string("\"quads1\":[\n")+quads1.toJSON()+string("],\n")
   +string("\"quads2\":[\n")+quads2.toJSON()+string("],\n")
   +string("\"quads3\":[\n")+quads3.toJSON()+string("],\n")
   +string("\"quads4\":[\n")+quads4.toJSON()+string("]\n")
  ;
 }
MANY(AtlasTile,AtlasTileHandle,AtlasTileHandles,"AtlasTile",AtlasTiles,{})
 MANYTOJSON(AtlasTile)
