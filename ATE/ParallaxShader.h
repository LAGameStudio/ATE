...
// Source: GLM-related tutorials #13 opengl-tutorial.org
class NormalMappingColorAlpha3LightShader : public GLSLShader {
public:
 GLMCamera camera;
 Vertex lightPosition1,lightPosition2,lightPosition3;
 float lightPower1,lightPower2,lightPower3;
 Crayon lightColor1,lightColor2,lightColor3;
 GLuint *diffuse,*normal;
 GLuint
  Ulight1,UlightPower1,UlightColor1,
  Ulight2,UlightPower2,UlightColor2,
  Ulight3,UlightPower3,UlightColor3,
  Umvp,Uv,Um,Umv3x3,Udiffuse,Udisplacement,Unormal; // color, gloss, normal
 GLuint
  UAposition,
  UAtexcoord,
  UAnormal,
  ...
  UAbitangent,
  UAcolor;
 Zp<VBOStaticVNTCTanBi> vbo;
 void Init() {
  diffuse=normal=null;
  Ulight1=UlightPower1=UlightColor1=
   Ulight2=UlightPower2=UlightColor2=
   Ulight3=UlightPower3=UlightColor3=
   Umvp=Uv=Um=Umv3x3=Udiffuse=Udisplacement=Unormal=0;
  UAposition=UAtexcoord=UAnormal=UAtangent=UAbitangent=UAcolor=0;
  lightPower1=10.0f;
  lightColor1.Pick(alabaster);
  lightPosition1.Set(0.0f,0.0f,3.0f);
  lightPower2=10.0f;
  ...
  lightColor3.Pick(alabaster);
  lightPosition3.Set(3.0f,3.0f,3.0f);
  OUTPUT("NormalMappingColorAlpha3LightShader:Loading\n");
  string vert=
   string("#version 120\n")+
   string("attribute vec3 position;\n")+
   string("attribute vec2 texcoord;\n")+
  ...
  string frag=
   ...
   string("varying vec4 VertexColor;\n")+
   string("void main() {\n")+
   string(" vec4 DM  = texture2D( DiffuseMap, UV ) * VertexColor;\n")+
   string(" vec3 Diff = DM.rgb;\n")+
   string(" vec3 Spec  = vec3(0.2);\n")+
   string(" vec3 Norm  = texture2D( NormalMap, UV ).rgb;\n")+
   string(" vec3 N = normalize(Norm*2.0 - 1.0);\n")+
   string(" vec3 E = normalize(EyeDirection_tangentspace);\n")+
   // Light 1
...
extern NormalMappingColorAlpha3LightShader normalmappingcoloralpha3lightShader;



// Source: GLM-related tutorials #13 opengl-tutorial.org
class IsoNormalMappingColorAlpha3LightShader : public GLSLShader {
public:
 GLMCamera camera;
 Vertex lightPosition1,lightPosition2,lightPosition3;
 float lightPower1,lightPower2,lightPower3;
 ...
 void Init() {
  ...
  lightColor3.Pick(alabaster);
  lightPosition3.Set(3.0f,3.0f,3.0f);
  OUTPUT("IsoNormalMappingColorAlpha3LightShader:Loading\n");
  string vert=
   string("#version 120\n")+
   string("attribute vec3 position;\n")+
   string("attribute vec2 texcoord;\n")+
...
extern IsoNormalMappingColorAlpha3LightShader isonormalmappingcoloralpha3lightShader;