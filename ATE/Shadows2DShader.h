...
public:
 ...
 void Init() {
  ...
  string frag=
   ...
   string("  }\n")+
   string(" }\n")+
   string(" gl_FragColor =lightColor*D*Brightness;\n")+
   string("}\n")
  ;
  loadShaders(frag.c_str());
  InitUniforms();
...
public:
 ...
 void Init() {
  ...
  string frag=
   ...
   +string("        {\n")
   +string("            shadow += clamp(texture2D(heightmap,v_vTexcoord * (1.0-(float(Z)*val)*(i))\n")
   ...
   +string("    gl_FragColor = mix( EM + CM * LM , AM, vec4(M) );\n")
   +string("}\n")
  ;
...
public:
  ...
  GLuint Upos,Upos1,Upos2,Upos3,Upos4,Upos5,Upos6,Upos7,Ucount,
         Uambience,Uemap,Uaddlightmap,Ulightmap,Ubacklight,Uheightmap,Ucolormap;
 void Init();
 void InitUniforms() {
  Upos=glGetUniformLocation(program, "pos");
