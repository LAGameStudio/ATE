...
GLImage *FastGUI::imagePicker(const char *label, GLImage *value,int x, int y, int w, int h, int bh,bool xToNull) {
 image(value,x,y,w,h,crayons.Pick(alabaster),transparency);
 fast.text(calibri,label,x+w/2,y+h+tfont.h);
 GetState(x,y,w,h);
 GLImage *result=value;
 bool wasClicked=clicked;
 if ( xToNull ) {
  if ( fast.smbutton("X",x+w-4-16,y+4,16,16) ) {
   result=null;
   wasClicked=false;
   ignoreNextRelease=1.0;
  }
 }
 if ( wasClicked ) {
  Zstring v;
  v=dialog_open("Choose an image file (PNG or JPG)");
  if ( v.ending(".png") || v.ending(".jpg") ) {
   GLImage *newImage=library.Load(v.c_str());
   if ( newImage ) result=newImage;
  }
 }
 return result;
...
//  Stretchi( rounded, hovering ? hover : color,    1.0f, none,     x,y,w,h );
 ...
 bool FastGUI::toggle( const char *labelTop, const char *labelBottom, const char *label, bool value, int  x, int y, int w, int h ) {
  ...
  Text(label,      ( hovering ? (clicking ? excited : highlight) : tint ),transparency,(double)(x+w)+fontSize.x,(double)y+((double)h/2)/2.0,fontSize.x,fontSize.y,charSpace,lineSpace,false);
...
/// numeric  ////////////////////////////////////////////////////////////////////////////////////////////
 ...
 double FastGUI::decimal( const char *label, double value, double lowest, double highest, double delta, FastExtents e, int characters, bool deltaOk, bool limitOk ) { return decimal(label,value,lowest,highest,delta,e.x,e.y,e.w,e.h,characters,deltaOk,limitOk); }
 double FastGUI::decimal( const char *label, double value, double lowest, double highest, double delta, int x, int y, int w, int h, int characters, bool deltaOk, bool limitOk ) {
  if ( deltaOk ) delta=number_delta(delta);
  int bw=w/2;
  bool allHovering = false;
  ...
  if ( limitOk ) wasnumber=wasnumber.substr(0,characters);
  string number=textbox( label, wasnumber, x + w + smallFontW, y, characters, limitOk );
  allHovering |= hovering;
  if ( allHovering ) {
   if ( input.wheelUp > 0 ) {
    value+=delta;
...
/// horizontal range select  ////////////////////////////////////////////////////////////////////////////////////////////
 ...
 void FastGUI::hrange( const char *label, double *value1, double *value2, double lowest, double highest, int x, int y, int w, int h, bool simple ) {
  Text(label,( hovering ? (clicking ? excited : highlight) : tint ),transparency,(double)(x),(double)y-(double)h-(double)h/8-fontSize.y,fontSize.x,fontSize.y,charSpace,lineSpace,false);
  ...
  rrect(
   ( hovering ? (clicking ? excited : highlight) : crayons.Pick(alabaster) ), (clicking?excited:color), (clicking?excited:color),
...
//  box(fg_E,newX,y-h,tenth,h*2+h/2);
 ...
 double FastGUI::hslider( const char *label, double value, double lowest, double highest, int x, int y, int w, int h, bool simple, bool nolabels) {
  ...
  if (!simple) {
   ...
   boxedtext(number.c_str(), (int)(newX), (int)(y + h / 2 - smallFontSize.y / 2), true, crayons.Pick(alabaster), crayons.Pick(ANSIboldgrey), crayons.Pick(ANSIbackgrey));
  } else {
   string number=
    !nolabels && tenth < 16 ? FORMAT("%d",(int)toReturn)
               : FORMAT("%.2f",toReturn);
   boxedtext(number.c_str(), (int)(newX), (int)(y + h / 2 - smallFontSize.y / 2), true,crayons.Pick(alabaster),crayons.Pick(ANSIboldgrey),crayons.Pick(ANSIbackgrey));
//   Text(number.c_str(),crayons.Pick(black),transparency,(double)(newX),(double)y+h/2,smallFontSize.x,smallFontSize.y,charSpace,lineSpace,true);
 ...
 int FastGUI::hslider( const char *label, int value, int lowest, int highest, int x, int y, int w, int h, bool simple, bool nolabels) {
  if (value<lowest) value = lowest; else if (value>highest) value = highest;
  GetState(x,y,w,h);
...
/// toggle list  ////////////////////////////////////////////////////////////////////////////////////////////
 ...
 int FastGUI::togglelist( TexFont *font, const char * label, Strings * values, int x, int y, int cw, int lines, int *scroll, int characterLimit, bool reorderable ) {
  Text(label,color,transparency,(double)(x+cw/2),(double)y-fontSize.y,fontSize.x,fontSize.y,charSpace,lineSpace,true);
  int h=(int)(lines*(tfont.h+7));
  //box(fg_C,x,y,cw,h);
  Blending(none);
  ...
  int rowHeight = (int)(tfont.h+5);
  Cartesian placement;
  Zp<String> wasHoveringItem;
  Cartesian hoveringWas;
  EACHN(values->first,String,s,{
   placement.SetRect(x,(int)(y+rowHeight*(i-*scroll)+5),cw,(int)(tfont.h));
   if ( (i-*scroll >=0 && i-*scroll < lines) ) {
    if ( hoverarea( placement.x+2,placement.y,placement.h,placement.h ) ) {
     hoveringWas.SetRect(placement.x+2,placement.y,placement.h,placement.h);
     wasHoveringItem=s;
    } else toggle("",s->toggled,placement.x+2,placement.y,placement.h,placement.h);
    Zstring formatted = s->s; int start=formatted.length-characterLimit; if ( start < 0 ) start=0; formatted=formatted(start,characterLimit);
    if ( reorderable ) {
     if ( s->prev && button(upArrow,placement.x+2+placement.h,placement.y,placement.h,placement.h) ) {
      values->sendUp(s);
      return -1;
     }
     if ( s->next && button(downArrow,placement.x+2+placement.h+placement.h,placement.y,placement.h,placement.h) ) {
      values->sendDown(s);
      return -1;
     }
    }
    if ( button(rounded,font,formatted.c_str(),placement.x+2+placement.h+(reorderable?placement.h*2:0),placement.y,placement.w-placement.h-2-(reorderable?placement.h*2:0),placement.h) ) {
     result = values->Element(s);
    }
   }
   i++;
  ...
  if ( wasHoveringItem ) {
   wasHoveringItem->toggled=toggle("",wasHoveringItem->toggled,hoveringWas.x,hoveringWas.y,hoveringWas.w,hoveringWas.h);
  }
  bool overflowing=values->count - lines > 0;
  GetState(x,y,cw,h);
  if ( overflowing && scroll && hovering ) {
   if ( input.wheelUp>0 ) {
    (*scroll)++;
   } else if ( input.wheelDown>0 ) {
    (*scroll)--;
...
/// tabs  ////////////////////////////////////////////////////////////////////////////////////////////
 ...
 string FastGUI::textbox( TexFont *font, Blends fontBlend, TexFont *labelFont, Blends labelFontBlend, const char *label, string value, int x, int y, int characters, bool limited, int lines, int lineSpace, int border, bool alwaysKeys, int kern, Zstring *goFullscreen, GLWindow *back_to, bool subcaption, bool textCloserMode, bool useTerminal ) {
  ...
  art.BevelRect2(rr_border, alwaysKeys ? excited : (hovering ? selected : deactivated),none,8.0,3.0,x,y,w,h);
...
//  mD.Render2dVC((float)x,(float)y,(float)w,(float)h+border);
 ...
 void FastGUI::arrows( const char *label, Cartesiand *xy, int x, int y, int bw, bool disabled, double delta ) {
  ...
  double factor=delta;
  boxedtext(FORMAT("%s\nx %1.1f\ny %1.1f", label, xy->x, xy->y).c_str(), (int)( (double)x + (double)bw*2.0 + 2.0 ), y, false, highlight);
  int ar=arrows("",x,y,bw,disabled,true,delta);
  if ( ar >= 0 ) {
   switch ( ar ) {
 ...
 void FastGUI::xy( Cartesiand *inOut, int x, int y, double xymin, double xymax, const char *label, const char *x_label, const char *y_label, int characters, bool limitOk ) {
  double delta=number_delta();
  Text(label,(double)x,(double)y+10.0,5.0,7.0,false);
  inOut->x=decimal(x_label,inOut->x,xymin,xymax,delta,x,y+16,32,16,characters,false,limitOk);
  inOut->y=decimal(y_label,inOut->y,xymin,xymax,delta,x+32+(fontSize.x+charSpace)*(characters+3),y+16,32,16,characters,false,limitOk);
 }
  
 void FastGUI::xy( Cartesian *inOut, int x, int y, int xymin, int xymax, const char *label ) {
  int delta=integer_delta();
  Text(label,(double)x,(double)y+10.0,5.0,7.0,false);
  inOut->x=integer("X",inOut->x,xymin,xymax,delta,x,y+16,32,16,7,false);
  inOut->y=integer("Y",inOut->y,xymin,xymax,delta,x+128,y+16,32,16,7,false);
 }

 void FastGUI::xy( const char *label, const char *label1, const char *label2, Cartesian *inOut, int x, int y, int xymin, int xymax ) {
  int delta=integer_delta();
  Text(label,(double)x,(double)y+10.0,5.0,7.0,false);
  inOut->x=integer(label1,inOut->x,xymin,xymax,delta,x,y+16,32,16,7,false);
  inOut->y=integer(label2,inOut->y,xymin,xymax,delta,x+128,y+16,32,16,7,false);
 }
 ...
 void FastGUI::xy( having precision, Cartesiand *inOut, int x, int y, double xymin, double xymax, const char *label ) {
  ...
  inOut->y=precise("y",inOut->y,xymin,xymax,delta,x,y+16,32,16,7,false);
 }
 ...
 void FastGUI::wh( Cartesiand *inOut, int x, int y, double xymin, double xymax, const char *label, const char *w_label, const char *h_label, int characters, bool limitOk ) {
  double delta=number_delta();
  Text(label,(double)x,(double)y+10.0,5.0,7.0,false);
  inOut->w=decimal(w_label,inOut->w,xymin,xymax,delta,x,y+16,32,16,characters,false,limitOk);
  inOut->h=decimal(h_label,inOut->h,xymin,xymax,delta,x+32+(fontSize.x+charSpace)*(characters+3),y+16,32,16,characters,false,limitOk);
 }
 ...
 void FastGUI::xyAngle( double *xx, double *yy, int x, int y, int maxX, int maxY ) {
  ...
  if (hovering && input.rightDepressed()) {
   (*xx) = 0.0;
   (*yy) = 0.0;
