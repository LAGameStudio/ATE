...
int CCommandLine::GetInt(char *Entry, int defaultValue) {
 Zstring entry = Entry;
 entry += '=';
 String *opt = Option((char *)entry.value.c_str());
 if (!opt) return defaultValue;
 Strings parts;
 parts.SplitPush(opt->s.c_str(),'=');
 Zstring second = parts.Get(1).c_str();
 return second.is_number() ? (int)second : defaultValue;
}

double CCommandLine::GetDecimal(char *Entry, double defaultValue) {
 Zstring entry = Entry;
 entry += '=';
 String *opt = Option((char *)entry.value.c_str());
 if (!opt) return defaultValue;
 ...
 parts.SplitPush(opt->s.c_str(), '=');
 Zstring second = parts.Get(1).c_str();
 return second.is_number() ? (double)second : defaultValue;
}