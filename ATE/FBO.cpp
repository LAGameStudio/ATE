...
FBOColorDepthStencil fboCDS1024, fboCDS1024b, fboCDS1024c, fboCDS2048, fboCDS2048b;
...
#include "Vertexd.h"
 ...
 void FBO::RenderQuad( Quad *q ) {
  ...
  glBegin(GL_QUADS);
  glTexCoord2i(0, 1);  glVertex2d(q->TL.x, q->TL.y);
  glTexCoord2i(1, 1);  glVertex2d(q->TR.x, q->TR.y);
  glTexCoord2i(1, 0);  glVertex2d(q->BR.x, q->BR.y);
  glTexCoord2i(0, 0);  glVertex2d(q->BL.x, q->BR.y);
  glEnd();
 ...
 void FBO::RenderQuadUpsideDown( Quad *q ) {
  ...
  glColor4fv(tint.floats); 
  glBegin(GL_QUADS);
  glTexCoord2i(0, 0);  glVertex2d(q->TL.x, q->TL.y);
  glTexCoord2i(1, 0);  glVertex2d(q->TR.x, q->TR.y);
  glTexCoord2i(1, 1);  glVertex2d(q->BR.x, q->BR.y);
  glTexCoord2i(0, 1);  glVertex2d(q->BL.x, q->BR.y);
  glEnd();
  glDisable(GL_TEXTURE_2D);
  glPopAttrib();
 }
...
void InitDefaultFBOs() {
 ...
 OUTPUT("fbo2048:");
 fbo2048.background.Pick(clear);
 fbo2048.blend = transparency;
 fbo2048.Create(2048, 2048);
 OUTPUT("fbo1024:");
 fbo1024.background.Pick(clear);
 fbo1024.blend = transparency;
 fbo1024.Create(1024, 1024);
 OUTPUT("fbo2048a:");
 fbo2048a.background.Pick(clear);
 fbo2048a.blend = transparency;
 fbo2048a.Create(2048, 2048);
 OUTPUT("fbo1024a:");
 fbo1024a.background.Pick(clear);
 fbo1024a.blend = transparency;
 fbo1024a.Create(1024, 1024);
 OUTPUT("fbo512:");
 ...
 fboCDS2048.Create(2048,2048);
 OUTPUT("fboCDS2kb:");
 fboCDS2048b.blend=transparency;
 fboCDS2048b.background.Pick(clear);
 fboCDS2048b.Create(2048,2048);
 OUTPUT("fbo1080p:");
...
// OUTPUT("fboCDS 1k, 2k:");
 ...
 void FBO::quad() {
  /*
  glPushMatrix();
  glScaled((double)w,(double)h,1.0);
  vboQuad.Render();
  glPopMatrix();
  */
  glBegin(GL_QUADS);
 ...
 void FBO::QuadXInverted() {
  ...
  glPushMatrix();
  glScaled((double)w,(double)h,1.0);
  vbo_QuadXInverted.Render();
  glPopMatrix();
  */
  glBegin(GL_QUADS);
  glTexCoord2i(1, 0); glVertex2i(0, 0);
  glTexCoord2i(0, 0); glVertex2i(w, 0);
  glTexCoord2i(0, 1); glVertex2i(w, h);
  glTexCoord2i(1, 1); glVertex2i(0, h);
  glEnd();
 }
 ...
 void FBO::quad( double stretchX, double stretchY ) {
  ...
  glBegin(GL_QUADS);
  glTexCoord2i(0, 0); glVertex2i(-xMag, -yMag);
  glTexCoord2i(1, 0); glVertex2i(xMag+w, -yMag);
  glTexCoord2i(1, 1); glVertex2i(xMag+w, yMag+h);
  glTexCoord2i(0, 1); glVertex2i(-xMag, yMag+h);
  glEnd();
 }
 ...
 void FBO::QuadXInverted( double stretchX, double stretchY ) {
  ...
  int yMag=(int)(stretchY*(double)h);
  glBegin(GL_QUADS);
  glTexCoord2i(1, 0); glVertex2i(-xMag, -yMag);
  glTexCoord2i(0, 0); glVertex2i(xMag+w, -yMag);
  glTexCoord2i(0, 1); glVertex2i(xMag+w, yMag+h);
  glTexCoord2i(1, 1); glVertex2i(-xMag, yMag+h);
  glEnd();
 }
 ...
 void FBO::QuadYInverted( double stretchX, double stretchY ) {
  ...
  int yMag=(int)(stretchY*(double)h);
  glBegin(GL_QUADS);
  glTexCoord2i(0, 1); glVertex2i(-xMag, -yMag);
  glTexCoord2i(1, 1); glVertex2i(xMag+w, -yMag);
  glTexCoord2i(1, 0); glVertex2i(xMag+w, yMag+h);
  glTexCoord2i(0, 0); glVertex2i(-xMag, yMag+h);
  glEnd();
 }
 ...
 void FBO::QuadXYInverted( double stretchX, double stretchY ) {
  ...
  int yMag=(int)(stretchY*(double)h);
  glBegin(GL_QUADS);
  glTexCoord2i(1, 1); glVertex2i(-xMag, -yMag);
  glTexCoord2i(0, 1); glVertex2i(xMag+w, -yMag);
  glTexCoord2i(0, 0); glVertex2i(xMag+w, yMag+h);
  glTexCoord2i(1, 0); glVertex2i(-xMag, yMag+h);
  glEnd();
 }
...
#define FBODRAWDEBUG(fbo,charstar,b,x,y) { Area(crayons.Pick(green255),(x),(y), (x)+64,(y)+64 ); crayons.Pick(black).gl(); Text((charstar), (x)+80-1,(y)+8+1, 8,10,true); crayons.Pick(alabaster).gl(); Text((charstar), (x)+80,(y)+8, 8,10,true); (fbo).blend=(b); (fbo).RenderUpsideDown((x),(y),(x)+64,(y)+64 ); Rectangle(crayons.Pick(black),(x),(y),(x)+64,(y)+64); }
...
void FBODrawDebug() {
 ...
 //FBODRAWDEBUG(fboCDSScreen,  "CDS0", transparency,512,138);
 ...
 FBODRAWDEBUG(fboCDS512,  "CDS5",  transparency,128+512,74 );
 ...
 FBODRAWDEBUG(fboCDS2048, "CDS2k",    transparency,256+512,138);
 FBODRAWDEBUG(fboCDS2048b, "CDS2kb",  transparency,256+512,202);
}
...
void FBODebugKeys() { 
 ...
 FBODEBUG(fboCDS2048b, transparency,256+512,332);
}
...
FBOColorDepthStencil *FBOColorDepthStencilfromString( const char *w ) {
 ...
 if ( !str_cmp(w,"CDS2048b") ) return &fboCDS2048b;
 return null;
}
 ...
 void FBO::RenderUpsideDown( double x, double y, double x2, double y2 ) {
  ...
  glReportError( glGetError() );
//  art.Stretch(texture,tint,blend,x,y,x2-x,y2-y);
  Blending(blend);
  glColor4fv(tint.floats);
  glEnable(GL_TEXTURE_2D);
  glBindTexture(GL_TEXTURE_2D, texture);
  ...
  glTexCoord2d(1.0,1.0);     glVertex2d(x2,y);
  glTexCoord2d(1.0,0.0);     glVertex2d(x2,y2);
  glTexCoord2d(0.0,0.0);     glVertex2d(x,y2);
  glEnd();
  glDisable(GL_BLEND);
  glDisable(GL_TEXTURE_2D);
