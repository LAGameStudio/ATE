...
public:
  ...
  */
  ...
  if ( winclass.hCursor == NULL ) OUTPUT("winclass.hCursor is NULL\n");
  ...
  mouseEscapes = CmdLine.Option("-mouseescape") || CmdLine.Option("-mouseescapes") || CmdLine.Option("-mouse-escapes");

  if (CmdLine.Option("-xofs")) {
   gl.xOfs = CmdLine.GetInt("-xofs", 0);
  }
  if (CmdLine.Option("-yofs")) {
   gl.yOfs = CmdLine.GetInt("-xofs", 0);
  }

  // Adjust Window, Account For Window Borders
  int xPos = GetSystemMetrics(SM_CXSCREEN) - display.w + gl.xOfs;
  int yPos = GetSystemMetrics(SM_CYSCREEN) - display.h + gl.yOfs;
  ...
  if (!(hwnd = CreateWindowEx(
   ...
   gl.noFullscreen && !CmdLine.Option("-recenter") ? xPos / 2 : gl.xOfs.value,
   gl.noFullscreen && !CmdLine.Option("-recenter") ? yPos / 2 : gl.yOfs.value,     // initial game window x,y
  ...
  if (gl.borderless || CmdLine.Option("-borderless") ) {
    ...
    SetWindowPos(hwnd, NULL, gl.xOfs, gl.yOfs, display.w, display.h, SWP_FRAMECHANGED | SWP_NOMOVE | SWP_NOSIZE | SWP_NOZORDER | SWP_NOOWNERZORDER);
  }
