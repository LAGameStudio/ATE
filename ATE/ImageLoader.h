...
//    case RAW: error=LoadRAW(  filename, &pixels, &w, &h ); break;
 ...
 CPUImage *RtoG() {
  ...
  for ( int i=0; i<w; i++ ) {
   for ( int j=0; j<h; j++ ) {
    GetPixel(i,j,&pix);
    pix.Double(pix.gd,pix.rd,pix.bd,pix.ad);
    out->SetPixel(w-i-1,j,&pix);
   }
  }
  return out;
 ...
 CPUImage *RtoB() {
  ...
  for ( int i=0; i<w; i++ ) {
   for ( int j=0; j<h; j++ ) {
    GetPixel(i,j,&pix);
    pix.Double(pix.bd,pix.gd,pix.rd,pix.ad);
    out->SetPixel(w-i-1,j,&pix);
   }
  }
  return out;
 ...
 CPUImage *GtoB() {
  ...
  for ( int i=0; i<w; i++ ) {
   for ( int j=0; j<h; j++ ) {
    GetPixel(i,j,&pix);
    pix.Double(pix.rd,pix.bd,pix.gd,pix.ad);
    out->SetPixel(w-i-1,j,&pix);
   }
  }
  return out;
 ...
 CPUImage *Half() {
  ...
  int h2=h/2;
  Crayon pix;
  CPUImage *out=new CPUImage(w/2,h/2);
  for ( int i=0; i<w2; i++ ) {
   for ( int j=0; j<h2; j++ ) {
    int px=i*2;
    int py=j*2;
 ...
 CPUImage *BlackToClear() { return ColorToClear(0,0,0); }
 CPUImage *MagentaToClear() { return ColorToClear(255,0,255); }
 CPUImage *ColorToClear( int transR, int transG, int transB ) {
  int w=getWidth();
  int h=getHeight();
  Crayon pix;
  CPUImage *out=new CPUImage(w,h);
  for ( int i=0; i<w; i++ ) {
   for ( int j=0; j<h; j++ ) {
    GetPixel(i,j,&pix);
    if ( pix.r == transR && pix.g == transG && pix.b == transB ) pix.Pick(clear);
    out->SetPixel(i,j,&pix);
   }
  }
  return out;
 }
 void Blit(CPUImage *in, int ofsX, int ofsY) {
  int endX = in->getWidth();
  int endY = in->getHeight();
  Crayon color;
  for (int x = 0; x < endX; x++) for (int y = 0; y < endY; y++) {
   int toX = x + ofsX;
   int toY = y + ofsY;
   if (!WITHIN(toX, toY, 0, 0, getWidth(), getHeight())) continue;
   in->GetPixel(x, y, &color);
   this->SetPixel(toX, toY, &color);
  }
 }
// CPUImage *Gaussian();
// CPUImage *HQ4X();
