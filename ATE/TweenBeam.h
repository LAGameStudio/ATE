...
public:
 ...
 void Init() {
  ...
  string frag=
   ...
   string("{\n")+
   string(" float ratio=gl_TexCoord[0].x;\n")+
   ...
   string(" vec3 color;\n")+
   string(" while ( i > 0 ) {\n")+
   string("  offset=vec2(gl_TexCoord[0].x*overlap/i,gl_TexCoord[0].y);\n")+
   ...
   string(" gl_FragColor = vec4(fc.r*ratio,fc.g*ratio,fc.b*ratio,1.0);\n")+
   string("}\n")
  ;
