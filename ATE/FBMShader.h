...
public:
 ...
 void Init() {
  ...
  string frag=
   ...
   string("uniform vec2 resolution,position;\n")+
   ...
   string("} \n")+
   string("void main() {\n")+
   string("    vec2 q = (gl_FragCoord.xy+position.xy) / resolution;\n")+
...
public:
 ...
 void Init() {
  a=0.25f;
  pTex0=null;
  OUTPUT("CheapShadowShader::Loading\n");
  string frag=
   string("#version 120\n")+
   string("#ifdef GL_ES\n")+
   ...
   string("void main() {\n")+
   string(" vec4 c=texture2D(tex0,gl_TexCoord[0].xy);\n")+
   string(" vec3 d=vec3(1.0-c.r,1.0-c.g,1.0-c.b);\n")+
   string(" gl_FragColor= vec4(d,c.r*a);\n")+
   string("}\n")
  ;
  loadShaders(frag.c_str());
  InitUniforms();
