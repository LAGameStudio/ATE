...
public:
 ...
 virtual TOJSON {
  Zstring filtered(s); filtered("\"","\\\"");
  return FORMAT( "\"%s\"", filtered.c_str() );
 }
};
...
public:
 ...
 TOJSON {
  Zstring filteredk(s); filteredk("\"","\\\"");
  Zstring filteredv(value); filteredv("\"","\\\"");
  return FORMAT( "\"%s\":\"%s\"", filteredk.c_str(), filteredv.c_str() );
 }
 virtual string toString() {
...
//Strings *ls( having recursive, const char *targetPath, bool listFiles=true, bool prependPath=true, bool filterOSFiles=true );

// Opens a Get Folder dialog, allows you to browse "This PC"
string dialog_get_folder( const char *msg1="", const char *msg2="Select folder directory from below:", bool newFolderButton=true );
...
// Opens the classic "Open.." dialog (with filtering)
string dialog_open( const char *msg1="", const char *msg2="Select file to read:", const char *allow="Portable Network Graphic (PNG)\n*.PNG\nJPEG Image (.jpg)\n*.jpg\nAll Files (*.*)\n*.*" );
// Opens the classic "Save As..." dialog
string dialog_saveas( const char *msg1="", const char *msg2="Select file to save:" );
...
void select_in_explorer( const char *s, Strings *selected=null );
