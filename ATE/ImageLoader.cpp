...
CPUImage *CPUImage::HeightToNormalMap() {
  ...
  for ( int i=0; i<w; i++ ) {
   for ( int j=0; j<h; j++ ) {
    if ( WITHIN(i,  j,  0,0,w-1,h-1) ) GetPixel(i,  j,  &p4); else { /* last value */ }
//    if ( WITHIN(i-1,j-1,0,0,w,h) ) GetPixel(i-1,j-1,&p0); else p0.fromCrayon(p4);
    if ( WITHIN(i,  j-1,0,0,w-1,h-1) ) GetPixel(i,  j-1,&p1); else p1.fromCrayon(p4);
//    if ( WITHIN(i+1,j-1,0,0,w,h) ) GetPixel(i+1,j-1,&p2); else p2.fromCrayon(p4);
    if ( WITHIN(i-1,j,  0,0,w-1,h-1) ) GetPixel(i-1,j,  &p3); else p3.fromCrayon(p4);
    if ( WITHIN(i+1,j,  0,0,w-1,h-1) ) GetPixel(i+1,j,  &p5); else p5.fromCrayon(p4);
//    if ( WITHIN(i-1,j+1,0,0,w,h) ) GetPixel(i-1,j+1,&p6); else p6.fromCrayon(p4);
    if ( WITHIN(i,  j+1,0,0,w-1,h-1) ) GetPixel(i,  j+1,&p7); else p7.fromCrayon(p4);
...
//     -(s6-s0+2.0*(s7-s1)+s8-s2),
    ...
    Crayon pix;
    pix.Int(
     (int) ((n.x + 1.0)/2.0 * 255.0), 
     (int) ((n.y + 1.0)/2.0 * 255.0),
     255,
     (int)(p4.ad * 255.0)
    );
