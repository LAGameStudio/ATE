...
// CPUImage *mipmap;
 ...
 // Basic constructor initializing empty GLImage
 GLImage(void);
 // Initializes an empty image of w x h and names it
 GLImage( const char *name, int w, int h );
 // Loads a GLImage content from a file, but does not upload it nor dispose of its CPU-side data, uses imageLoader, generates no mipmaps
 GLImage( const char * filename );
 // Loads a GLImage content from a file using imageLoader, does not upload it nor dispose of its CPU-side data, generates no mipmaps
 GLImage( string filename );
 // Wraps the GLImage class around a provided CPUImage, does not upload it, nor dispose of the CPU-side data, generates no mipmaps
 GLImage( CPUImage *in );
 // Wraps the GLImage class around a provided CPUImage, does not upload it, nor dispose of the CPU-side data, generates no mipmaps, names it
 GLImage( const char *name, CPUImage *in );
 // Directly uploads an rgb_buffer of w/h named name, generating a texture, and does not hold the rgb_buffer pointer
 GLImage( float *rgb_buffer, int w, int h, const char *name, GLint mag, GLint min, GLint wrapS, GLint wrapT, bool mipmaps=true );
 // Directly uploads an rgba_buffer of w/h named name, generating a texture, and does not hold the rgb_buffer pointer
...
// void Save( char *filename );
 ...
 void Upload( GLSetup *glSetup=&gl, GLint magFilter=GL_LINEAR, GLint minFilter=GL_LINEAR_MIPMAP_LINEAR, GLint wrapS=GL_CLAMP_TO_EDGE, GLint wrapT=GL_CLAMP_TO_EDGE );
 void Unload(void);
