...
public:
 ...
 void Init() {
  ...
  string frag=
   ...
   string("float[9] GetData(in int channel) { float dxtex = 1.0 / float(textureSize(Texture0,0)); float dytex = 1.0 / float(textureSize(Texture0,0)); float[9] mat; int k = -1; for (int i=-1; i<2; i++) { for(int j=-1; j<2; j++) { k++; mat[k] = get_pixel(gl_TexCoord[0].xy,float(i)*dxtex, float(j)*dytex)[channel]; } } return mat; }\n")+
   ...
   string("gl_FragColor = vec4(Convolve(k,mata,1.,1./2.), Convolve(k,mata,1.,1./2.), Convolve(k,mata,1.,1./2.),1.0);\n")+
   string("}")
   ;
...
public:
 ...
 void Init() {
  ...
  string frag=
   ...
   string("float[9] k = float[] (2.,0.,0., 0., -1., 0., 0., 0., -1.);\n")+
   string("float matr[9] = GetData(0); float matg[9] = GetData(1); float matb[9] = GetData(2); float mata[9] = GetMean(matr,matg,matb);\n")+
   string("gl_FragColor = vec4(Convolve(k,mata,1.,1./2.), Convolve(k,mata,1.,1./2.), Convolve(k,mata,1.,1./2.),Convolve(k,mata,1.,1./2.));\n")+
   string("}")
   ;
  loadShaders(frag.c_str());
  InitUniforms();
...
public:
 ...
 void Init() {
  ...
  string frag=
   ...
   string("float[9] GetMean(in float[9] matr, in float[9] matg, in float[9] matb) { float[9] mat; for (int i=0; i<9; i++) { mat[i] = (matr[i]+matg[i]+matb[i])/3.; } return mat; }\n")+
   string("void main(void) {\n")+
   string("vec4 here=texture2D(Texture0,gl_TexCoord[0].xy);\n")+
   string("float[9] k = float[] (2.,0.,0., 0., -1., 0., 0., 0., -1.);\n")+
   string("float matr[9] = GetData(0); float matg[9] = GetData(1); float matb[9] = GetData(2); float mata[9] = GetMean(matr,matg,matb);\n")+
   string("gl_FragColor = vec4(Convolve(k,mata,1.,1./2.), Convolve(k,mata,1.,1./2.), Convolve(k,mata,1.,1./2.),here.a);\n")+
   string("}")
   ;
  loadShaders(frag.c_str());
  InitUniforms();
...
public:
 ...
 void Init() {
  ...
  string frag=
   ...
   string("float matr[9] = GetData(0); float matg[9] = GetData(1); float matb[9] = GetData(2); float mata[9] = GetMean(matr,matg,matb);\n")+
   string("gl_FragColor = vec4(Convolve(k,matr,1.,0.), Convolve(k,matg,1.,0.), Convolve(k,matb,1.,0.),1.0);\n")+
   string("}")
   ;
  loadShaders(frag.c_str());
  InitUniforms();
...
public:
 ...
 void Init() {
  ...
  string frag=
   ...
   string("float matr[9] = GetData(0); float matg[9] = GetData(1); float matb[9] = GetData(2); float mata[9] = GetMean(matr,matg,matb);\n")+
   string("gl_FragColor = vec4(Convolve(k,matr,16.,0.), Convolve(k,matg,16.,0.), Convolve(k,matb,16.,0.),1.0);\n")+
   string("}")
   ;
  loadShaders(frag.c_str());
  InitUniforms();
...
public:
 ...
 void Init() {
  ...
  string frag=
   ...
   string("float matr[9] = GetData(0); float matg[9] = GetData(1); float matb[9] = GetData(2); float mata[9] = GetMean(matr,matg,matb);\n")+
   string("gl_FragColor = vec4(Convolve(k,mata,0.1,0.), Convolve(k,mata,0.1,0.), Convolve(k,mata,0.1,0.),1.0);\n")+
   string("}")
   ;
  loadShaders(frag.c_str());
  InitUniforms();
...
//#if defined(VERTICAL_BLUR_9)
   ...
   string(" avgValue += texture2D(blurSampler, gl_TexCoord[0].xy) * incrementalGaussian.x;\n")+
   ...
   string("   avgValue += texture2D(blurSampler, gl_TexCoord[0].xy - i * blurSize * \n")+
   ...
   string("   incrementalGaussian.xy *= incrementalGaussian.yz;\n")+
   string(" }\n")+
   string(" gl_FragColor = avgValue / coefficientSum;\n")+
   string("}")
 ;
  loadShaders(frag.c_str());
  InitUniforms();
...
public:
 ...
 void Init() {
  ...
  string frag=
   ...
   string(" for ( int i=0; i<9; i++) sample[i]=texture2D(tex0,gl_TexCoord[0].st+ofs[i]);\n")+
   string(" gl_FragColor=(sample[4]*8.0)-(sample[0]+sample[1]+sample[2]+sample[3]+sample[5]+sample[6]+sample[7]+sample[8]);\n")+
   string("}\n")
  ;
  loadShaders(frag.c_str());
  InitUniforms();
...
public:
 ...
 void Init() {
  ...
  string frag=
   ...
   string(" glFragColor.rgb=sqrt((He.rgb*He.rgb) + (Ve.rgb*Ve.rgb));\n")+
   string(" glFragColor.a=1.0;\n")+
   string("}\n")
  ;
  loadShaders(frag.c_str());
  InitUniforms();
...
//   string("}\n")+
   ...
   string("gl_FragColor = vec4( packNormal(n.x), packNormal(n.y), packNormal(1.0), 0.0 );\n")+
   string("}\n")
  ;
//  OUTPUT(frag.c_str());
  loadShaders(frag.c_str());
...
public:
 ...
 void Init() {
  ...
  string frag=
   ...
   string("	float s4 = texture2D(tex0, vec2(tc.x, tc.y + dV)).r;\n")+
   string(" vec2 n=normalize( vec2( (s1-s3), (s2-s4) ) ) * coef;\n")+
   string("gl_FragColor = vec4( packNormal(n.x), packNormal(n.y), packNormal(1.0), tc.r );\n")+
   string("}\n")
  ;
//  OUTPUT(frag.c_str());
  loadShaders(frag.c_str());
