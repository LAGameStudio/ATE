...
public:
 ...
 void Load() {
  string file=specialPaths.historic("ate.options.txt");
  if ( !file_exists(file.c_str()) ) ResetToDefaults();
  else fromString(file_as_string(file.c_str()).c_str());
  ApplyVolume();
 }
 void Save() {
  string_as_file(toString().c_str(),specialPaths.historic("ate.options.txt").c_str());
 }
 void ApplyVolume() {
...
public:
 ...
 GameOptionsPanel() : MoveableWindowManager() {
  ...
  statefile=specialPaths.historic("ate.options.panel.windows.txt");
 }
 void LoadEditors() {
