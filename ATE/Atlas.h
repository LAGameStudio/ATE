...
ONE(AtlasFrame,{})
 ...
 TOJSON {
  return FORMAT(
   JSONTREE("extents")
   JSONTREE("sourceSize")
   JSONTREE("spriteSourceSize")
   JSONTREE("pivot")
   JSONBOOL("texturePacker")
   JSONBOOL("trimmed")
   JSONBOOL_("rotated"),
   extents.toJSON().c_str(),
   sourceSize.toJSON().c_str(),
   spriteSourceSize.toJSON().c_str(),
   pivot.toJSON().c_str(),
   JSONTF(texturePacker),
   JSONTF(trimmed),
   JSONTF(rotated)
  );
 }
...
ONE(Atlas,{})
 ...
 void Create( GLImage *source, int rows, int cols ) {
  this->texture=source;
  if ( texture ) this->textureName=texture->filename;
  if ( textureName.begins(specialPaths.working_directory.c_str()) ) textureName(specialPaths.working_directory.c_str(),"");
  textureName("\\","/");
  if ( height ) this->heightName=height->filename;
  if ( heightName.begins(specialPaths.working_directory.c_str()) ) heightName(specialPaths.working_directory.c_str(),"");
  heightName("\\","/");
  if ( normal ) this->normalName=normal->filename; 
  if ( normalName.begins(specialPaths.working_directory.c_str()) ) normalName(specialPaths.working_directory.c_str(),"");
  normalName("\\","/");
  uniformGrid=true;
...
//    individuals.Append(coords(i,j).Duplicate());
 ...
 void RenderPreview( int x, int y, int w, int h ) {
  art.Stretch(texture,crayons.Pick(alabaster),transparency,x,y,w,h);
 }
 void GenElements();
 ...
 WORDKEYS({
  ...
  WORDKEY("height",heightName)
  WORDKEY("normal",normalName)
 ...
 TOJSON {
  return FORMAT(
   ...
   (int)columns,
   frames.toJSON().c_str(),
   centerOffsets.toJSON().c_str(),
   JSONTF(dontGenElements)
  );
 }
 JSONINDEXEDNUMBER(elements_toJSON,elements)
MANY(Atlas,AtlasHandle,AtlasHandles,"Atlas",AltasLibrary,{})
 MANYTOJSON(Atlas)
 KEYSWORDSGroups("Atlas",Atlas)
  void Load( AtlasHandles *handles, const char *p ) {
   if ( file_exists(p) ) {
    Atlas *wasLast=(Atlas *)last;
    fromString(file_as_string(p).c_str());
    if ( wasLast ) {
     EACH(wasLast->next,Atlas,a) handles->Add(a);
    } else {
     EACH(first,Atlas,a) handles->Add(a);
    }
   }
  }
 void Save( AtlasHandles *handles, const char *filename ) {
  Zstring out;
  EACH(handles->first,AtlasHandle,ah) {
   out+=string("Atlas {")+ah->p->toString()+string("}\n");
  }
  string_as_file(out.c_str(),filename);
 }
DONE(Atlas);
...
ONE(AtlasFontGlyph,{c='\0';})
 ...
 TOJSON {
  Zstring filteredLetter; filteredLetter("\"","\\\"");
  return FORMAT(
   JSONTREE("texcoords")
   JSONTREE("offset")
   JSONTREE("finetune")
   ...
   JSONSTR("letter")
   JSONCHAR("c"),
   texCoords.toJSON().c_str(),
   offsetSize.toJSON().c_str(),
   finetune.toJSON().c_str(),
   (int)ord,
   filteredLetter.c_str(),
   c
  );
 }
MANY(AtlasFontGlyph,AtlasFontGlyphHandle,AtlasFontGlyphHandles,"AtlasFontGlyph",AtlasFontGlyphLibrary,{})
 MANYTOJSON(AtlasFontGlyph)
...
//  if ( fitTo1 ) quads.FitTo1();
 ...
 TOJSON {
  return FORMAT(
   ...
   JSONARRAY("legend")
   JSONDECIMAL("base")
   JSONDECIMAL("spaceWidth")
   JSONDECIMAL("lineHeight")
   JSONDECIMAL("kern")
   JSONINT("rows")
   JSONINT_("columns"),
   name.c_str(),
