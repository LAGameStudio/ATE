...
bool SavePNG( string filename, int w, int h, Indexed<GLubyte> *pixels ) {
 ...
 if(result < 0) {
  OUTPUT("The PNG encoder is not installed.  Using JPEG fallback.\n");
 ...
 for ( int x=0; x<w; x++ ) for ( int y=0; y<h; y++ ) {
  pix.Int(
   (int) pixels->list[(x+y*w)*4+0],
   (int) pixels->list[(x+y*w)*4+1],
   (int) pixels->list[(x+y*w)*4+2],
   (int) pixels->list[(x+y*w)*4+3]
  );
  color.SetValue(color.MakeARGB((BYTE)pix.a,(BYTE)pix.r,(BYTE)pix.g,(BYTE)pix.b));
