...
void ShadedBlend3Way( GLuint *mixer, GLuint *tR, GLuint *tG, GLuint *fallback,
                      FBOColor *out );
...
void ShadedBlend5Way( GLuint *mixer, GLuint *tR, GLuint *tG, GLuint *tB,
                      GLuint *tBlack, GLuint *fallback, FBOColor *out );
...
public:
 ...
 void Init() {
  ...
  string frag=
   ...
   string("void main() {\n")+
   string(" vec4 canvas=texture2D(tex0,gl_TexCoord[0].xy);\n")+
   string(" vec2 tcpaint=gl_TexCoord[0].xy;\n")+
   string(" tcpaint.y=1.0-tcpaint.y;\n")+
   string(" vec4 paint=texture2D(tex1,tcpaint.xy);\n")+
   string(" float pavg=(paint.r+paint.g+paint.b)/3.0;\n")+
   string(" gl_FragColor=vec4(mix(canvas.rgb,paint.rgb*canvas.rgb,min(pavg,paint.a)*factor),canvas.a);\n")+
   string("}\n")
  ;
...
public:
 ...
 void Init() {
  ...
  string frag=
   ...
   string(" vec4 paint=texture2D(tex1,tcpaint.xy);\n")+
   string(" float pavg=(paint.r+paint.g+paint.b)/3.0;\n")+
   string(" gl_FragColor=vec4(mix(canvas.rgb,paint.rgb,min(paint.a,pavg)*factor),canvas.a);\n")+
   string("}\n")
  ;
  loadShaders(frag.c_str());
  InitUniforms();
...
public:
 ...
 void Init() {
  ...
  string frag=
   ...
   string(" tcpaint.y=1.0-tcpaint.y;\n")+
   string(" vec4 paint=texture2D(tex1,tcpaint.xy);\n")+
   string(" gl_FragColor=vec4(mix(paint.rgb,canvas.rgb,paint.a),canvas.a);\n")+
   string("}\n")
  ;
  loadShaders(frag.c_str());
  InitUniforms();
...
public:
 ...
 void Init() {
  ...
  string frag=
   ...
   string(" tcpaint.y=1.0-tcpaint.y;\n")+
   string(" vec4 paint=texture2D(tex1,tcpaint.xy);\n")+
   string(" gl_FragColor=vec4(mix(canvas.rgb,paint.rgb,paint.a*factor),canvas.a);\n")+
   string("}\n")
  ;
  loadShaders(frag.c_str());
  InitUniforms();
...
public:
 ...
 void Init() {
  ...
  string frag=
   ...
   string("void main() {\n")+
   string(" vec4 canvas=texture2D(tex0,gl_TexCoord[0].xy);\n")+
   string(" vec4 paint=texture2D(tex1,gl_TexCoord[0].xy);\n")+
   string(" gl_FragColor=vec4(mix(canvas.rgb,paint.rgb,paint.a*factor),canvas.a);\n")+
   string("}\n")
  ;
  loadShaders(frag.c_str());
...
public:
 ...
 void Init() {
  ...
  string frag=
   ...
   string("uniform sampler2D tex0;")+
   string("void main() {\n")+
   string(" vec4 sample=texture2D(tex0,gl_TexCoord[0].xy);\n")+
   string(" if ( sample.r > threshold.r && sample.g > threshold.g && sample.b > threshold.b ) gl_FragColor=replace;\n")+
   string(" else gl_FragColor=sample.rgba;\n")+
   string("}\n")
  ;
  loadShaders(frag.c_str());
  InitUniforms();
...
public:
 ...
 void Init() {
  ...
  string frag=
   ...
   string("void main() {\n")+
   string(" vec4 sample=texture2D(tex0,gl_TexCoord[0].xy);\n")+
   string(" if ( sample.r > threshold ) gl_FragColor=vec4(0.0,0.0,sample.r,1.0);\n")+
   string(" else gl_FragColor=vec4(sample.rgb,(sample.r+sample.g+sample.b)/3.0);\n")+
   string("}\n")
  ;
  loadShaders(frag.c_str());
  InitUniforms();
...
public:
 ...
 void Init() {
  ...
  string frag=
   ...
   string(" vec4 sample=texture2D(tex0,gl_TexCoord[0].xy);\n")+
   string(" if ( sample.r < threshold.r && sample.g < threshold.g && sample.b < threshold.b ) gl_FragColor=replace;\n")+
   string(" else gl_FragColor=sample;\n")+
   string("}\n")
  ;
  loadShaders(frag.c_str());
  InitUniforms();
...
public:
 ...
 void Init() {
  ...
  string frag=
   ...
   string("void main() {\n")+
   string(" vec4 sample=texture2D(tex0,gl_TexCoord[0].xy);\n")+
   string(" if ( sample.r > thresholdA ) gl_FragColor=vec4((sample.r-thresholdA)/(1.0-thresholdA),0.0,0.0,1.0);\n")+
   string(" else if ( sample.r > thresholdB ) gl_FragColor=vec4(0.0,(sample.g-thresholdB)/(thresholdA-thresholdB),0.0,1.0);\n")+
   string(" else gl_FragColor=vec4(0.0,0.0,sample.b/thresholdB,1.0);\n")+
   string("}\n")
  ;
  loadShaders(frag.c_str());
  InitUniforms();
...
public:
 ...
 void Init() {
  ...
  string frag=
   ...
   string("void main() {\n")+
   string(" vec4 sample=texture2D(tex0,gl_TexCoord[0].xy);\n")+
   string(" if ( sample.r < threshold.r && sample.g < threshold.g && sample.b < threshold.b ) gl_FragColor=below;\n")+
   string(" else gl_FragColor=above;\n")+
   string("}\n")
  ;
  loadShaders(frag.c_str());
  InitUniforms();
...
public:
 ...
 void Init() {
  ...
  string frag=
   ...
   string("void main() {\n")+
   string(" vec4 mixer=texture2D(tex0,gl_TexCoord[0].xy);\n")+
   string(" vec4 sampleR=texture2D(tex1,gl_TexCoord[0].xy);\n")+
   string(" vec4 sampleG=texture2D(tex2,gl_TexCoord[0].xy);\n")+
   string(" vec4 sampleB=texture2D(tex3,gl_TexCoord[0].xy);\n")+
   string(" gl_FragColor=(sampleR*mixer.r+sampleG*mixer.g+sampleB*mixer.b);\n")+
   string("}\n")
  ;
  loadShaders(frag.c_str());
  InitUniforms();
...
public:
 ...
 void Init() {
  ...
  string frag=
   ...
   string(" vec4 sampleG=texture2D(tex2,gl_TexCoord[0].xy);\n")+
   string(" vec4 sampleB=texture2D(tex3,gl_TexCoord[0].xy);\n")+
   string(" gl_FragColor=(sampleR*mixer.r+sampleG*mixer.g+sampleB*mixer.b)/3.0;\n")+
   string("}\n")
  ;
  loadShaders(frag.c_str());
  InitUniforms();
...
public:
 ...
 void Init() {
  ...
  string frag=
   ...
   string("void main() {\n")+
   string(" vec4 sample=texture2D(tex0,gl_TexCoord[0].xy);\n")+
   string(" gl_FragColor=vec4(sample.rgb,(sample.r+sample.g+sample.b)/3.0);\n")+
   string("}\n")
  ;
  loadShaders(frag.c_str());
  InitUniforms();
...
public:
 ...
 void Init() {
  ...
  string frag=
   ...
   string("void main() {\n")+
   string(" vec4 mixer=texture2D(tex0,gl_TexCoord[0].xy);\n")+
   string(" vec4 sample=texture2D(tex1,gl_TexCoord[0].xy);\n")+
   string(" gl_FragColor=vec4(sample.rgb*mixer.rgb,mixer.a);\n")+
   string("}\n")
  ;
  loadShaders(frag.c_str());
  InitUniforms();
...
public:
 ...
 void Init() {
  ...
  string frag=
   ...
   string(" vec4 mixer=texture2D(tex0,gl_TexCoord[0].xy);\n")+
   string(" vec4 sample=texture2D(tex1,gl_TexCoord[0].xy);\n")+
   string(" gl_FragColor=vec4(sample.rgb,mixer.a);\n")+
   string("}\n")
  ;
  loadShaders(frag.c_str());
  InitUniforms();
...
public:
 ...
 void Init() {
  ...
  string frag=
   ...
   string(" vec4 mixer=texture2D(tex0,gl_TexCoord[0].xy);\n")+
   string(" vec4 sample=texture2D(tex1,gl_TexCoord[0].xy);\n")+
   string(" gl_FragColor=vec4(sample.rgb,mixer.a*sample.a);\n")+
   string("}\n")
  ;
  loadShaders(frag.c_str());
  InitUniforms();
...
public:
 ...
 void Init() {
  ...
  string frag=
   ...
   string(" vec4 mixer=texture2D(tex0,gl_TexCoord[0].xy);\n")+
   string(" vec4 sample=texture2D(tex1,gl_TexCoord[0].xy);\n")+
   string(" gl_FragColor=vec4(sample.rgb,1.0-mixer.a);\n")+
   string("}\n")
  ;
  loadShaders(frag.c_str());
  InitUniforms();
...
public:
 ...
 void Init() {
  ...
  string frag=
   ...
   string("void main() {\n")+
   string(" vec4 mixer=texture2D(tex0,gl_TexCoord[0].xy);\n")+
   string(" if ( mixer.a == 0.0 ) gl_FragColor=texture2D(tex1,gl_TexCoord[0].xy);\n")+
   string(" else gl_FragColor=vec4(0.0,0.0,0.0,0.0);\n")+
   string("}\n")
  ;
  loadShaders(frag.c_str());
  InitUniforms();
...
public:
 ...
 void Init() {
  ...
  string frag=
   ...
   string("void main() {\n")+
   string(" vec4 mixer=texture2D(tex0,gl_TexCoord[0].xy);\n")+
   string(" vec4 bg=texture2D(tex2,gl_TexCoord[0].xy);\n")+
   string(" vec4 res;\n")+
...
public:
 ...
 void Init() {
  ...
  string frag=
   ...
   string("void main() {\n")+
   string(" vec4 mixer=texture2D(tex0,gl_TexCoord[0].xy);\n")+
   string(" vec4 bg=texture2D(tex3,gl_TexCoord[0].xy);\n")+
   string(" vec4 res;\n")+
   string(" if ( mixer.r == 1.0 ) {\n")+
   string("  vec4 sample=texture2D(tex2,gl_TexCoord[0].xy);\n")+
   string("  res=mix(bg,sample,mixer.a);\n")+
...
public:
 ...
 void Init() {
  ...
  string frag=
   ...
   string("void main() {\n")+
   string(" vec4 mixer=texture2D(tex0,gl_TexCoord[0].xy);\n")+
   string(" vec4 bg=texture2D(tex4,gl_TexCoord[0].xy);\n")+
   string(" vec4 res;\n")+
   string(" if ( mixer.r == 1.0 ) {\n")+
   string("  vec4 sample=texture2D(tex3,gl_TexCoord[0].xy);\n")+
   string("  res=mix(bg,sample,mixer.a);\n")+
   string(" } else if ( mixer.g == 1.0 ) {\n")+
   string("  vec4 sample=texture2D(tex2,gl_TexCoord[0].xy);\n")+
   string("  res=mix(bg,sample,mixer.a);\n")+
...
public:
 ...
 void Init() {
  ...
  string frag=
   ...
   string("void main() {\n")+
   string(" vec4 mixer=texture2D(tex0,gl_TexCoord[0].xy);\n")+
   string(" vec4 bg=texture2D(tex5,gl_TexCoord[0].xy);\n")+
   string(" vec4 res;\n")+
   string(" if ( mixer.r == 1.0 ) {\n")+
   string("  vec4 sample=texture2D(tex4,gl_TexCoord[0].xy);\n")+
   string("  res=mix(bg,sample,mixer.a);\n")+
   string(" } else if ( mixer.g == 1.0 ) {\n")+
   string("  vec4 sample=texture2D(tex3,gl_TexCoord[0].xy);\n")+
   string("  res=mix(bg,sample,mixer.a);\n")+
...
public:
 ...
 void Init() {
  ...
  string frag=
   ...
   string("void main() {\n")+
   string(" vec4 mixer=texture2D(tex0,gl_TexCoord[0].xy);\n")+
   string(" vec4 bg=texture2D(tex6,gl_TexCoord[0].xy);\n")+
   string(" vec4 res;\n")+
   string(" if ( mixer.r == 1.0 ) {\n")+
   string("  vec4 sample=texture2D(tex5,gl_TexCoord[0].xy);\n")+
   string("  res=mix(bg,sample,mixer.a);\n")+
   string(" } else if ( mixer.g == 1.0 ) {\n")+
   string("  vec4 sample=texture2D(tex4,gl_TexCoord[0].xy);\n")+
   string("  res=mix(bg,sample,mixer.a);\n")+
...
public:
 ...
 void Init() {
  ...
  string frag=
   ...
   string("void main() {\n")+
   string(" vec4 mixer=texture2D(tex0,gl_TexCoord[0].xy);\n")+
   string(" vec4 bg=texture2D(tex7,gl_TexCoord[0].xy);\n")+
   string(" vec4 res;\n")+
   string(" if ( mixer.r == 1.0 ) {\n")+
   string("  vec4 sample=texture2D(tex6,gl_TexCoord[0].xy);\n")+
   string("  res=mix(bg,sample,mixer.a);\n")+
   string(" } else if ( mixer.g == 1.0 ) {\n")+
   string("  vec4 sample=texture2D(tex5,gl_TexCoord[0].xy);\n")+
   string("  res=mix(bg,sample,mixer.a);\n")+
...
public:
 ...
 void Init() {
  ...
  string frag=
   ...
   string("void main() {\n")+
   string(" vec4 mixer=texture2D(tex0,gl_TexCoord[0].xy);\n")+
   string(" vec4 bg=texture2D(tex8,gl_TexCoord[0].xy);\n")+
   string(" vec4 res;\n")+
   string(" if ( mixer.r == 1.0 && mixer.b == 1.0 ) {\n")+
   string("  vec4 sample=texture2D(tex7,gl_TexCoord[0].xy);\n")+
   string("  res=mix(bg,sample,mixer.a);\n")+
   string(" } else if ( mixer.r == 1.0 && mixer.g == 1.0 ) {\n")+
   string("  vec4 sample=texture2D(tex6,gl_TexCoord[0].xy);\n")+
   string("  res=mix(bg,sample,mixer.a);\n")+
