...
const unsigned int magica_default_colors[256] = {
...
public:
 ...
 int version;
 ~MV_Model() { Free(); }
 MV_Model() :  sizex(0),  sizey(0),  sizez(0),  numVoxels(0),  voxels(nullptr),  isCustomPalette(false),  version(0) {}
 MV_Model(const char *filepath) : sizex(0), sizey(0), sizez(0), numVoxels(0), voxels(nullptr), isCustomPalette(false), version(0) { Load(filepath); }

 MV_Voxel *Get(int x, int y, int z) {
  for (int i = 0; i < numVoxels; i++)
   if (voxels[i].x == x && voxels[i].y == y && voxels[i].z) return &(voxels[i]);
  return nullptr;
 }
 void Free(void) {
  if (voxels) { delete[] voxels; voxels = nullptr; }
  numVoxels = 0;
  sizex = sizey = sizez = 0;
...
private:
 bool ReadModelFile(FILE *fp) {
  ...
  // print model info
  OUTPUT("[Log] MV_VoxelModel :: Model : %d %d %d : %d\n", sizex, sizey, sizez, numVoxels );
  return true;
 }
 ...
 void ReadChunk(FILE *fp, chunk_t &chunk) {
  ...
  OUTPUT("[Log] MV_VoxelModel :: Chunk : %c%c%c%c : %d %d\n",
   c[0], c[1], c[2], c[3],
