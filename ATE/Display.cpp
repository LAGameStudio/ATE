...
void Display::CommandLine() {
 ...
 if (CmdLine.Option("-1360x768")) {
   w = 1360;
   h = 768;
   OUTPUT("Setting 1360x768 as the resolution.\n");
 } else
 ...
 if (CmdLine.Option("-x=") || CmdLine.Option("-y=")) {
  w = CmdLine.GetInt("-x", GetSystemMetrics(SM_CXSCREEN));
  w = CmdLine.GetInt("-y", GetSystemMetrics(SM_CYSCREEN));
 }
 else {
  ...
  if ( _res.length() > 0 ) {
   ...
   OUTPUT("Forcing resolution to %d by %d (%f)",a,b,iratiod(a,b));
   w=a;
   h=b;
