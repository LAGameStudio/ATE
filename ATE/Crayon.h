...
// Crayon(Crayon c) { fromCrayon(c); } // can't do this one
 ...
 Crayon RGBtoYUV() {
  Crayon yuv;
  yuv.rd= 0.299 * rd + 0.587 * gd + 0.114 * bd; // Y'
  yuv.gd=-0.147 * rd - 0.289 * gd + 0.436 * bd; // U
  yuv.bd= 0.615 * rd - 0.515 * gd - 0.100 * bd; // V
  yuv.Double(yuv.rd,yuv.gd,yuv.bd,ad);
  return yuv;
 }

 Crayon DiffRGBasYUV( Crayon a, Crayon b ) {
  Crayon yuv1=a.RGBtoYUV();
  Crayon yuv2=b.RGBtoYUV();
  Crayon result;
  result.Int(
   ABS(yuv1.r - yuv2.r) > 0x00300000 ? 1 : 0,
   ABS(yuv1.g - yuv2.g) > 0x00000700 ? 1 : 0,
   ABS(yuv1.b - yuv2.b) > 0x00000006 ? 1 : 0,
   255
  );
  return result;
 }
 ...
 void Multiply(float f, having preservedAlpha) {
  Float(f*rf, f*gf, f*bf, af);
 }
 void Divide( float f ) {
